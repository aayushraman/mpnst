...
H3K79me2 <- dba(sampleSheet = "H3K79me2.csv")
...
## All transcripts
reads.RPKM.mat.transcription <- reads.RPKM.mat

## Top regions
ind <- which(reads.RPKM.mat.transcription <  0)
reads.RPKM.mat.transcription[ind] <- 0
hmcols <- colorRampPalette(brewer.pal(9,"GnBu"))(100)
all.peaks <- varFilter(eset = reads.RPKM.mat.transcription, var.cutoff = 0.75) ## Top 10000 peaks
all.peaks <- CancerSubtypes::FSbyMAD(Data = reads.RPKM.mat.transcription, cut.type = "topk", value = 10000)
print(dim(all.peaks))
plot(H3K79me2_reads_minus)
...
## GREAT
file <- "H3K79me2"
file <- paste("~/Desktop/H3K79me2/",file,"All_diff.feat.selected.txt",sep = "") ## change the directory if you want
bedfile <- data.frame()
...
write.table(x = background[,2:4], file = "~/Desktop/H3K79me2/All_Background.txt", sep="\t", quote=F, 
...
write.table(x = peaks_up, file = "~/Desktop/H3K79me2/Peaks_PRC2Neg-PRC2WT_up.txt", 
            sep="\t", quote=F, row.names = FALSE, col.names = TRUE)
write.table(x = peaks_up[,c(1:3)], file = "~/Desktop/H3K79me2/Peaks_PRC2Neg-PRC2WT_up-GREAT.bed", 
            sep="\t", quote=F, row.names = FALSE, col.names = FALSE)
dim(peaks_up)
write.table(x = peaks_down, file = "~/Desktop/H3K79me2/Peaks_PRC2Neg-PRC2WT_down.txt", 
            sep="\t", quote=F, row.names = FALSE, col.names = TRUE)
write.table(x = peaks_down[,c(1:3)], file = "~/Desktop/H3K79me2/Peaks_PRC2Neg-PRC2WT_down-GREAT.bed", 
            sep="\t", quote=F, row.names = FALSE, col.names = FALSE)
dim(peaks_down)
