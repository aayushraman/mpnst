...
H3K79me2 <- dba(sampleSheet = "H3K79me2_K27me3loss.csv")
H3K79me2_reads_minus <- dba.count(H3K79me2, fragmentSize = 200, score = DBA_SCORE_READS_MINUS)
save(H3K79me2_reads_minus, file = "H3K79me2_reads_minus.RData")
load("/Volumes/My_Passport/MPNST_cell_lines_compiled_Chip_seq_data/04aln_downsample/H3K79me2_reads_minus.RData")

## from dba count object
reads.RPKM <- dba.peakset(H3K79me2_reads_minus, bRetrieve = TRUE, DataType = DBA_DATA_FRAME)
name <- "RPKM"
...
colnames(reads.RPKM.mat) <- as.vector(H3K79me2_reads_minus$class["Tissue",])
genotypes <- factor(c(rep("PRC2 loss",5),rep("PRC2 WT", 4)), levels = c("PRC2 loss", "PRC2 WT"))
sample.lab <- as.vector(H3K79me2_reads_minus$class["Tissue",])

## All Promoters
reads.RPKM.mat.genebody <- reads.RPKM.mat

## Top regions
ind <- which(reads.RPKM.mat.genebody <  0)
reads.RPKM.mat.genebody[ind] <- 0
...
all.peaks <- CancerSubtypes::FSbyMAD(Data = reads.RPKM.mat.genebody, cut.type = "topk", value = 10000)
print(dim(all.peaks))
plot(H3K79me2_reads_minus)
...
annot1$genotypes <- factor(annot1$genotypes, levels = c("PRC2loss", "PRC2WT"))
...
## heatmap
annot1 <- data.frame(genotypes, row.names = sample.lab)
annot1$genotypes <- factor(annot1$genotypes, levels = c("PRC2 WT", "PRC2 loss"))
pheatmap(log2(all.peaks+1), cluster_rows = TRUE, cluster_cols = TRUE, show_rownames = FALSE,
         ...
         annotation = annot1)
dba.plotHeatmap(H3K79me2_reads_minus, correlations=FALSE)

## GREAT
file <- "H3K79me2"
file <- paste("~/Desktop/",file,"All_diff.feat.selected.txt",sep = "") ## change the directory if you want
bedfile <- data.frame()
...
## Supervised Heatmaps
H3K79me2_contrast <- dba.contrast(H3K79me2_reads_minus, 
                                  H3K79me2_reads_minus$masks$PRC2loss,
                                  H3K79me2_reads_minus$masks$PRC2WT,
                                    "PRC2loss", "PRC2WT") ## CHANGE
H3K79me2_analyze <- dba.analyze(H3K79me2_contrast)
dba.report(H3K79me2_analyze)
plot(H3K79me2_analyze, contrast=1)
plot(H3K79me2_analyze, contrast=1, correlations=FALSE)

degs <- data.frame(dba.report(H3K79me2_analyze))
rownames(degs) <- paste(degs$seqnames,":",degs$start,"-",degs$end,sep = "")

dba.plotHeatmap(H3K79me2_analyze, correlations=FALSE, contrast = 1)
plot(H3K79me2_analyze, contrast=1)
mat <- plot(H3K79me2_analyze, contrast=1, correlations=FALSE)
mat <- data.frame(mat)
...
annot1 <- data.frame(genotypes, row.names = sample.lab)
annot1$genotypes <- factor(annot1$genotypes, levels = c("PRC2 WT", "NSC", "PRC2 loss"))
colnames(mat)[13:21] <- sample.lab
rownames(mat) <- mat$region
pheatmap(mat[,13:21], cluster_rows = TRUE, cluster_cols = TRUE, show_rownames = FALSE,
         show_colnames = TRUE, fontsize_row = 9, legend = TRUE, filename = NA,
...
peaks <- data.frame(dba.report(H3K79me2_analyze))
peaks_up <- peaks[which(peaks$Fold > 0 & peaks$FDR < 0.05),]
peaks_down <- peaks[which(peaks$Fold < 0 & peaks$FDR < 0.05),]
write.table(x = peaks_up, file = "~/Desktop/Peaks_PRC2Neg-PRC2WT_up.txt", 
            sep="\t", quote=F, row.names = FALSE, col.names = TRUE)
write.table(x = peaks_up[,c(1:3)], file = "~/Desktop/Peaks_PRC2Neg-PRC2WT_up-GREAT.bed", 
