...
H3K4me3_reads_minus <- dba.count(H3K4me3, fragmentSize = 200, score = DBA_SCORE_READS_MINUS)
save(H3K4me3_reads_minus, file = "H3K4me3_reads_minus.RData")
load("/Volumes/My_Passport/MPNST_cell_lines_compiled_Chip_seq_data/04aln_downsample/H3K4me3_reads_minus.RData")
reads.RPKM <- dba.peakset(H3K4me3_reads_minus, bRetrieve = TRUE, DataType = DBA_DATA_FRAME)

plot(H3K4me3_reads_minus)
...
colnames(reads.RPKM.mat) <- as.vector(H3K4me3_reads_minus$class["Tissue",])
genotypes <- factor(c(rep("PRC2loss",5),rep("PRC2WT", 4), rep("NSC")), levels = c("PRC2loss", "PRC2WT", "NSC"))
sample.lab <- as.vector(H3K4me3_reads_minus$class["Tissue",])
...
pheatmap(log2(all.peaks+1), cluster_rows = TRUE, cluster_cols = TRUE, show_rownames = FALSE,
         ...
         annotation = annot1)
dba.plotHeatmap(H3K4me3_reads_minus, correlations=FALSE)

## GREAT
file <- "H3K4me3"
file <- paste("~/Desktop/H3K4me3/",file,"All_diff.feat.selected.txt",sep = "") ## change the directory if you want
bedfile <- data.frame()
...
write.table(x = background[,2:4], file = "~/Desktop/H3K4me3/All_Background.txt", sep="\t", quote=F, 
            row.names = FALSE, col.names = FALSE)

### Supervised Heatmaps from Differential Analysis ###

## Contrast, analyze and report from DiffBind ##
H3K4me3_contrast <- dba.contrast(H3K4me3_reads_minus, 
                                 H3K4me3_reads_minus$masks$PRC2loss,
                                 H3K4me3_reads_minus$masks$PRC2WT,
                                 "PRC2loss", "PRC2WT") ## CHANGE
H3K4me3_analyze <- dba.analyze(H3K4me3_contrast)
dba.report(H3K4me3_analyze)
plot(H3K4me3_analyze, contrast=1)
plot(H3K4me3_analyze, contrast=1, correlations=FALSE)
peaks <- data.frame(dba.report(H3K4me3_analyze))
peaks_up <- peaks[which(peaks$Fold > 0 & peaks$FDR < 0.05),]
peaks_down <- peaks[which(peaks$Fold < 0 & peaks$FDR < 0.05),]
write.table(x = peaks_up, file = "~/Desktop/H3K4me3/Peaks_PRC2Neg-PRC2WT_up.txt", 
            sep="\t", quote=F, row.names = FALSE, col.names = TRUE)
write.table(x = peaks_up[,c(1:3)], file = "~/Desktop/H3K4me3/Peaks_PRC2Neg-PRC2WT_up-GREAT.bed", 
            sep="\t", quote=F, row.names = FALSE, col.names = FALSE)
dim(peaks_up)
write.table(x = peaks_down, file = "~/Desktop/H3K4me3/Peaks_PRC2Neg-PRC2WT_down.txt", 
            sep="\t", quote=F, row.names = FALSE, col.names = TRUE)
write.table(x = peaks_down[,c(1:3)], file = "~/Desktop/H3K4me3/Peaks_PRC2Neg-PRC2WT_down-GREAT.bed", 
            sep="\t", quote=F, row.names = FALSE, col.names = FALSE)
dim(peaks_down)

## Supervised Heatmaps
dba.plotHeatmap(H3K4me3_analyze, correlations=FALSE, contrast = 1)
