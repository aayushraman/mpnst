...
H3K4me1 <- dba(sampleSheet = "H3K4me1_K27me3loss.csv")
H3K4me1_K27me3loss_reads_minus <- dba.count(H3K4me1, fragmentSize = 200, score = DBA_SCORE_READS_MINUS)
save(H3K4me1_K27me3loss_reads_minus, file = "H3K4me1_K27me3loss_reads_minus.RData")
load("/Volumes/My_Passport/MPNST_cell_lines_compiled_Chip_seq_data/04aln_downsample/H3K4me1_K27me3loss_reads_minus.RData")
reads.RPKM <- dba.peakset(H3K4me1_K27me3loss_reads_minus, bRetrieve = TRUE, DataType = DBA_DATA_FRAME)

plot(H3K4me1_K27me3loss_reads_minus)
...
colnames(reads.RPKM.mat) <- as.vector(H3K4me1_K27me3loss_reads_minus$class["Tissue",])
genotypes <- factor(c(rep("PRC2loss",5),rep("PRC2WT", 4)), levels = c("PRC2loss", "PRC2WT"))
sample.lab <- as.vector(H3K4me1_K27me3loss_reads_minus$class["Tissue",])

## All heterochromatin
reads.RPKM.mat.enhancer <- reads.RPKM.mat

## Top regions
ind <- which(reads.RPKM.mat.enhancer <  0)
reads.RPKM.mat.enhancer[ind] <- 0
hmcols <- colorRampPalette(brewer.pal(9,"GnBu"))(100)
all.peaks <- varFilter(eset = reads.RPKM.mat.enhancer, var.cutoff = 0.75) ## Top 10000 peaks
all.peaks <- CancerSubtypes::FSbyMAD(Data = reads.RPKM.mat.enhancer, cut.type = "topk", value = 12000)
print(dim(all.peaks))
plot(H3K4me1_K27me3loss_reads_minus)
...
annot1$genotypes <- factor(annot1$genotypes, levels = c("PRC2WT", "PRC2loss"))
pheatmap(log2(all.peaks+1), cluster_rows = TRUE, cluster_cols = TRUE, show_rownames = FALSE,
         ...
         annotation = annot1)
dba.plotHeatmap(H3K4me1_K27me3loss_reads_minus, correlations=FALSE)
...
H3K4me1_K27me3loss_contrast <- dba.contrast(H3K4me1_K27me3loss_reads_minus, 
                                            H3K4me1_K27me3loss_reads_minus$masks$PRC2loss,
                                            H3K4me1_K27me3loss_reads_minus$masks$PRC2WT,
                                            "PRC2loss", "PRC2WT") ## CHANGE
H3K4me1_K27me3loss_analyze <- dba.analyze(H3K4me1_K27me3loss_contrast)
dba.report(H3K4me1_K27me3loss_analyze)
plot(H3K4me1_K27me3loss_analyze, contrast=1)
plot(H3K4me1_K27me3loss_analyze, contrast=1, correlations=FALSE)
peaks <- data.frame(dba.report(H3K4me1_K27me3loss_analyze))
...
## Supervised Heatmaps
dba.plotHeatmap(H3K4me1_K27me3loss_analyze, correlations=FALSE, contrast = 1)
